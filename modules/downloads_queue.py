...
class DownloadsQueue():
	...
	async def initiate(self, download_id: int, message_id: int, last_message: str) -> None:
		task = await self.bot.db.update_download( download_id, {'last_message':last_message,'message_id':message_id,'status':DOWNLOAD_STATUS.INIT} )
		if task:
	...
	async def __queue_step(self) -> None:
		...
		if queue_moved:
			...
			_waiting_ids_len = len(_waiting_ids)
			for download_id in _waiting_ids:
				await self.__queue_moved(download_id,_waiting_ids_len)

		await asyncio.sleep(self.bot.config.DOWNLOADS_CHECK_INTERVAL)
	
	async def __queue_add(self, task: Download) -> None:
	...
	async def __queue_moved(self, download_id: int, _waiting_ids_len: int) -> None:
		...
		if position != self._queue[download_id]['position']:
			...
			message = f'Позиция в очереди: {position}/{_waiting_ids_len}'
